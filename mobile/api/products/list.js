...
async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const { data: member } = await supabase.from('restaurant_members').select('restaurant_id').eq('user_id', req.user.id).maybeSingle();
    if (!member) return res.status(400).json({ error: 'Restaurante no encontrado' });
    // Delete primero para hacer upsert manual sin necesitar constraint UNIQUE
    await supabase.from('supplier_aliases').delete().eq('alias', alias.trim()).eq('restaurant_id', member.restaurant_id);
    const { error } = await supabase
      .from('supplier_aliases')
      .insert({ alias: alias.trim(), canonical: canonical.trim(), restaurant_id: member.restaurant_id });
  ...
  if (req.method === 'DELETE') {
    ...
    const { data: member } = await supabase.from('restaurant_members').select('restaurant_id').eq('user_id', req.user.id).maybeSingle();
    if (!member) return res.status(400).json({ error: 'Restaurante no encontrado' });
    const { error } = await supabase.from('supplier_aliases').delete().eq('alias', alias.trim()).eq('restaurant_id', member.restaurant_id);
    if (error) return res.status(500).json({ error: error.message });
    return res.status(200).json({ success: true });
  }
  ...
  try {
    ...
    if (grouped) {
      ...
      const { data: memberRow } = await supabase.from('restaurant_members').select('restaurant_id').eq('user_id', req.user.id).maybeSingle();
      let aliasQuery = supabase.from('supplier_aliases').select('alias, canonical');
      if (memberRow?.restaurant_id) aliasQuery = aliasQuery.eq('restaurant_id', memberRow.restaurant_id);
